...
from columnar_transposition import decrypt_columnar_transposition_b, encrypt_columnar_transposition_b, \
    decrypt_columnar_transposition_a, encrypt_columnar_transposition_a
...
class MainPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.top_frame = tk.Frame(self, bg='gray22', width=600, height=100, pady=5, padx=5)
        ...
        self.logo = tk.Label(self.top_frame, text="BSK APP", bg='gray22',
                             fg='gray63', width="100", height="2", font=("Verdana", 20, 'bold')).pack()
...
class Menu(tk.Frame):
    def __init__(self, parent):
        ...
        button1 = tk.Button(self, text="Implementacja podstawowych modułów kryptograficznych", fg='gray68', bg='gray14',
                            font=('calibri', 10, 'bold'), height=3,
...
class BasicCryptographyFrame(tk.Frame):
    def __init__(self, parent):
        ...
        self.option_label = tk.Label(self.option_frame, text="OPCJE", bg='gray70',
                             fg='gray21', font=("calibri", 11, 'bold')).pack()
        ...
        for option, val in options:
            tk.Radiobutton(self.option_frame,
                           text=option,
                           padx=20,
                           variable=self.coded_or_decoded,
                           command=ShowChoice,
                           value=val, bg="gray70").pack(anchor=tk.W)

        sep = tk.Frame(self.option_frame, width=1, bg='gray30')
        sep.pack(fill='x')


        ciphers = ["Rail Fence", "Przestawienie macierzowe A", "Przestawienie macierzowe B"]

        self.cipher_label = tk.Label(self.option_frame, text="Wybierz metode szyfrowania: ", bg='gray70',
                                     fg='gray16').pack()
        self.cipher = tk.StringVar()
        cipher_choosen = ttk.Combobox(self.option_frame, textvariable=self.cipher, state="readonly", width=25)
        cipher_choosen['values'] = ciphers
        cipher_choosen.pack(pady=10)
        cipher_choosen.current(0)
        open_file_button = tk.Button(self.option_frame, text="wczytaj plik tekstowy",
                                     width=20, command=lambda: self.open_file()).pack()
        save_file_button = tk.Button(self.option_frame, text="zapisz jako plik tekstowy",
                                     width=20, bg='grey21', fg='snow', command=lambda: self.save_file()).pack()
        ...
        button = tk.Button(self.data_frame, text="Szyfruj / Deszyfruj ", width=20, command=lambda: self.decrypt_encrypt())
